...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["ç"],
  description: "YouTube'den Şarkı Veya Video Oynatır !",
  async execute(message, args) {
    const { channel } = message.member.voice;

    const serverQueue = message.client.queue.get(message.guild.id);
    if (!channel) return message.reply("İlk Önce Ses Kanalına Girmen Gerekli ! ").catch(console.error);
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return message.reply(`İle aynı kanalda olmalısınız ${message.client.user}`).catch(console.error);

    if (!args.length)
      return message
        .reply(`Kullanım: ${message.client.prefix}play <YouTube URL | Video İsmi >`)
        .catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("Bağlantı"))
