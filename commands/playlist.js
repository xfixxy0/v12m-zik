...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Kullanım: ${message.client.prefix}playlist <YouTube Playlist URL'si | Playlist İsmi>`)
        .catch(console.error);
    if (!channel) return message.reply("İlk Önce Ses Kanalına Giremen Gerekiyor").catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("BAĞLANTI"))
      return message.reply("Ses kanalına bağlanılamıyor, izinler eksik");
    if (!permissions.has("KONUŞMAK"))
    ...
    if (urlValid) {
      try {
        videos = await playlist.getVideos(MAX_PLAYLIST_SIZE || 10, { part: "pasaj" });
      } catch (error) {
        console.error(error);
      ...
      try {
        const results = await youtube.searchPlaylists(search, 1, { part: "pasaj" });
        playlist = results[0];
        videos = await playlist.getVideos(MAX_PLAYLIST_SIZE || 10, { part: "pasaj" });
      } catch (error) {
        console.error(error);
