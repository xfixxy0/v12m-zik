...
module.exports = {
  async play(song, message) {
    ...
    if (!song) {
      ...
      return queue.textChannel.send("🚫 Müzik Bitti.").catch(console.error);
    }
    ...
    try {
      var playingMessage = await queue.textChannel.send(`🎶 Şuan Oynatılıyor: **${song.title}** ${song.url}`);
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔁":
          ...
          queue.textChannel.send(`Döngü ${queue.loop ? "**açık**" : "**kapalı**"}`).catch(console.error);
          break;
